...
# formate simulated spectra like test set data
mixedDf <- readRDS("../temp/complexFeatures.rds") %>%
    rename(Ab = AbPres, Kp = KpPres) %>%
    mutate(Ab = fct_relevel(Ab, fctOrder),
           Kp = fct_relevel(Kp, fctOrder))

mixedAb <- mixedDf %>% select(Ab, starts_with("mz"))
mixedKp <- mixedDf %>% select(Kp, starts_with("mz"))

mixedList <- list(Ab = mixedAb, Kp = mixedKp)
mixedDmatList <- prepareData(mixedList, orgLabs = c("Ab", "Kp"))

# get results from model predictions

# Ab
mixedAbRes <- formatResults(abMod, mixedDmatList$Ab) %>%
    cbind(select(mixedDf, id, n, AbCoeff, KpCoeff, Ab, Kp))
