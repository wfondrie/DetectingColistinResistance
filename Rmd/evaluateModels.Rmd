...
plotCurve <- function(dat, type = "ROC", title = NULL) {
    # Main Plotting
    p <- ggplot(dat, aes(x = V1, y = V2, color = classifier))
...
nnRes <- readRDS("../temp/nnRes.rds")
```

#### Function to Format ROC and PR results
```{r formatCurveResults}
combineResults <- function(mlResults, sfResults, nnResults, type = "species") {
    ...
    nn <- as.tibble(nnResults[[type]]$curve) %>% mutate(classifier = "Nearest Neighbor")
    
    ml %>% rbind(sf) %>% rbind(nn)
}
```
...
```{r plotAb}
abTitleSpec <- expression(italic("A. baumannii")~"(Species)")
abTitleRes <- expression(italic("A. baumannii")~"(Resistance)")

# Species ROC
abSpecRoc <- combineResults(abRoc, sfRes$sfAbRoc, nnRes$nnAbRoc, type = "species")
plotCurve(abSpecRoc, type = "ROC", title = abTitleSpec)
ggsave("../results/AbSpecRocCurve.pdf", width = 70, height = 50, units = "mm", useDingbats = F)

# Resistance ROC
abPosRoc <- combineResults(abRoc, sfRes$sfAbRoc, nnRes$nnAbRoc, type = "pos")
plotCurve(abPosRoc, type = "ROC", title = abTitleRes)
ggsave("../results/AbResRocCurve.pdf", width = 70, height = 50, units = "mm", useDingbats = F)

# Species PR
abSpecPr <- combineResults(abPr, sfRes$sfAbPr, nnRes$nnAbPr, type = "species")
plotCurve(abSpecPr, type = "PR", title = abTitleSpec)
ggsave("../results/AbSpecPrCurve.pdf", width = 70, height = 50, units = "mm", useDingbats = F)

# Resistance PR
abPosPr <- combineResults(abPr, sfRes$sfAbPr, nnRes$nnAbPr, type = "pos")
plotCurve(abPosPr, type = "PR", title = abTitleRes)
ggsave("../results/AbResPrCurve.pdf", width = 70, height = 50, units = "mm", useDingbats = F)
```

#### Klebsiella ROC & PR Curves
```{r plotKp}
kpTitleSpec <- expression(italic("K. pneumoniae")~"(Species)")
kpTitleRes <- expression(italic("K. pneumoniae")~"(Resistance)")

# Species ROC
kpSpecRoc <- combineResults(kpRoc, sfRes$sfKpRoc, nnRes$nnKpRoc, type = "species")
plotCurve(kpSpecRoc, type = "ROC", title = kpTitleSpec)
ggsave("../results/KpSpecRocCurve.pdf", width = 70, height = 50, units = "mm", useDingbats = F)

# Resistance ROC
kpPosRoc <- combineResults(kpRoc, sfRes$sfKpRoc, nnRes$nnKpRoc, type = "pos")
plotCurve(kpPosRoc, type = "ROC", title = kpTitleRes)
ggsave("../results/KpResRocCurve.pdf", width = 70, height = 50, units = "mm", useDingbats = F)

# Species PR
kpSpecPr <- combineResults(kpPr, sfRes$sfKpPr, nnRes$nnKpPr, type = "species")
plotCurve(kpSpecPr, type = "PR", title = kpTitleSpec)
ggsave("../results/KpSpecPrCurve.pdf", width = 70, height = 50, units = "mm", useDingbats = F)

# Resistance PR
kpPosPr <- combineResults(kpPr, sfRes$sfKpPr, nnRes$nnKpPr, type = "pos")
plotCurve(kpPosPr, type = "PR", title = kpTitleRes)
ggsave("../results/KpResPrCurve.pdf", width = 70, height = 50, units = "mm", useDingbats = F)
```
