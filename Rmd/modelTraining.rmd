...
files <- list.files("../data/fullLib", 
                    ...
                    pattern = "mzXML$",
                    recursive = T)
files <- sample(files, 200) # uncomment for testing
...
paramGrid <- expand.grid(max_depth = 5,
                        min_child_weight = 4,
                        gamma = 0)
...
finalParams <- bstParams %>%
                group_by(org) %>%
                do(optimizeXgbParams(., dmatList[[.$org]]$dat, 0.3))
...
testdmatList <- prepareData(testDatList, orgLabs = c("Ab", "Kp"))


saveRDS(testdmatList, "../temp/testDataList.rds")
```